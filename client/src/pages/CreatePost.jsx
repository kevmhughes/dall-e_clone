...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if(form.prompt) {
      try {
          ...
          const response = await fetch("https://dall-e-clone-xcf1.onrender.com/api/v1/dalle", {
            method: "POST",
            headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if(form.prompt && form.photo) {
      setLoading(true);
      try {
        const response = await fetch("https://dall-e-clone-xcf1.onrender.com/api/v1/post", {
          method: "POST",
          headers: {
            'Content-Type': 'application/json'
          },
